#!/usr/bin/env python3
"""Print or validate coconut_vault store release-note output paths."""
...
def validate_locale_directories(root: Path, flavor: str) -> list[str]:
    ...
    for platform, expected in expected_by_platform.items():
        flavor_directory = generated / platform / flavor
        if not flavor_directory.is_dir():
            continue

        for path in sorted(flavor_directory.iterdir()):
            if path.is_dir() and not path.name.startswith(".") and path.name not in expected:
                errors.append(f"unsupported locale directory: {path.relative_to(root)}")
...
def main() -> int:
    ...
    errors = validate_source(source) + validate_outputs(outputs) + validate_locale_directories(root, args.flavor)
    if errors:
        for error in errors:
            print(f"ERROR: {error}", file=sys.stderr)
        return 1
