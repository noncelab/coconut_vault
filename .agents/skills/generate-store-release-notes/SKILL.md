...
11. Before writing, ensure the target flavor contains only the locale directories listed in [references/locales.md](references/locales.md). Remove unsupported locale directories only when they are empty. If an unsupported locale directory contains files, stop and report it instead of deleting its contents.
12. Write UTF-8 files with one trailing newline to every planned iOS and Android path. Use identical text for the corresponding iOS and Android language; only locale directory names differ. Never create output for an unlisted locale.
...
14. Report the flavor, Android version code, generated paths, and validation result. Remind the user to review translations before deployment.
