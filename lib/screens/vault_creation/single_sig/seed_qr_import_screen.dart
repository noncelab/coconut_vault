...
class _SeedQrImportScreenState extends State<SeedQrImportScreen> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: CoconutAppBar.build(context: context, title: '시드 QR 코드 스캔'),
      body: QRView(
        key: qrKey,
  ...
  void _onQRViewCreated(QRViewController controller) {
    ...
    controller.scannedDataStream.listen((scanData) {
      ...
      print('words: $words');
      // todo: 니모닉 확인 화면 및 패스프레이즈 사용 화면으로 이동
  ...
  List<String> _decodeCompactQR(List<int> bytes) {
    ...
    final bitStream = _toBitStream(bytes);
    final totalBits = wordCount * 11;
    final usefulBits = bitStream.sublist(12);
  ...
  List<int> _toBitStream(List<int> bytes) {
    final bits = <int>[];
