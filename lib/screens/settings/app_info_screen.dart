...
class _AppInfoScreenState extends State<AppInfoScreen> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        backgroundColor: _isScrollOverTitleHeight ? CoconutColors.white.withValues(alpha: 0.2) : CoconutColors.white,
        ...
        title: AnimatedOpacity(
          ...
          child: Text(t.app_info, style: CoconutTypography.heading4_18),
        ),
        centerTitle: true,
      ),
          ...
          SingleButton(
            ...
            onPressed: () {
              MyBottomSheet.showBottomSheet_90(
                context: context,
                child: const TutorialScreen(screenStatus: TutorialScreenStatus.modal),
              );
            },
          ),
          ...
          ButtonGroup(
            buttons: [
              ...
              SingleButton(
                ...
                leftElement: ClipRRect(
                  borderRadius: BorderRadius.circular(8.0),
                  child: Image.asset('assets/png/discord-full-logo.png', width: 24, height: 24, fit: BoxFit.cover),
                ),
                onPressed: () {
                  MyBottomSheet.showBottomSheet_90(
                    context: context,
              ...
              SingleButton(
                ...
                leftElement: ClipRRect(
                  borderRadius: BorderRadius.circular(8.0),
                  child: Image.asset('assets/jpg/x-logo.jpg', width: 24, height: 24, fit: BoxFit.cover),
                ),
                onPressed: () {
                  MyBottomSheet.showBottomSheet_90(
                    context: context,
                    child: QrcodeBottomSheet(qrData: X_POW, title: t.app_info_screen.ask_to_x, fromAppInfo: true),
                  );
                },
              ),
              SingleButton(
                ...
                leftElement: ClipRRect(
                  borderRadius: BorderRadius.circular(8.0),
                  child: Image.asset('assets/png/mail-icon.png', width: 24, height: 24, fit: BoxFit.cover),
                ),
                onPressed: () async {
                  ...
                  MyBottomSheet.showBottomSheet_90(
                    context: context,
                    child: QrcodeBottomSheet(
                      qrData: 'mailto:$CONTACT_EMAIL_ADDRESS?subject=${t.email_subject}}&body=$info',
                      title: t.app_info_screen.ask_to_email,
                      fromAppInfo: true,
                    ),
                  );
  ...
  Widget githubWidget() {
    Widget githubLogo = SvgPicture.asset('assets/svg/github-logo.svg', width: 24, height: 24, fit: BoxFit.cover);
    return Container(
      padding: const EdgeInsets.symmetric(horizontal: 16),
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
  ...
  Widget footerWidget(Future<PackageInfo> packageInfoFuture) {
    return FutureBuilder<PackageInfo>(
      ...
      builder: (BuildContext context, AsyncSnapshot<PackageInfo> snapshot) {
        if (snapshot.connectionState == ConnectionState.waiting) {
          return Center(child: CircularProgressIndicator(color: CoconutColors.white.withValues(alpha: 0.06)));
        } else if (snapshot.hasError) {
          return Center(child: Text(t.errors.data_loading_error));
        } else if (!snapshot.hasData) {
          return Center(child: Text(t.errors.data_not_found_error));
