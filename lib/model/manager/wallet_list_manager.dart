...
class WalletListManager {
  ...
  Future loadAndEmitEachWallet(List<dynamic> jsonList,
    ...
    for (int i = 0; i < jsonList.length; i++) {
      ...
      if (jsonList[i][vaultTypeField] == VaultType.singleSignature.name) {
        ...
        item = SinglesigVaultListItem.fromJson(jsonList[i]);
      } else {
        item = MultisigVaultListItem.fromJson(jsonList[i]);
      }
  ...
  Future<SinglesigVaultListItem> addSinglesigWallet(
    ...
    final singleWallet = SinglesigWallet.fromJson(vaultData);

    SinglesigVaultListItem newItem = SinglesigVaultListItem(
        id: singleWallet.id!,
        name: singleWallet.name!,
        colorIndex: singleWallet.color!,
        iconIndex: singleWallet.icon!,
        secret: singleWallet.mnemonic!,
        passphrase: singleWallet.passphrase!);

    _linkNewSinglesigVaultAndMultisigVaults(newItem);
