## mainnet 3.2.0, regtest 5.2.0 - 2026-09-18
### 개선
1. MFP 4글자씩 띄워서 표시
### 수정
1. 키스톤으로 내보내는 멀티 시그 지갑 BSMS QR: 기기 화면 크기에 따라 Animated QR로 전환
### 기타
1. 학습용 앱 아이콘 수정

## mainnet 3.0.3, regtest 5.0.3 - 2026-06-24
* Account Index 변경 시 발생하는 오류 수정
* 자잘한 UI 버그 수정

## mainnet 3.0.2, regtest 5.0.2 - 2026-06-22
• 코코넛 라이브러리 변경사항 적용

## mainnet 3.0.1, regtest 5.0.1 - 2026-06-19
### Fixed
* 탭루트 지갑 생성 과정의 문구, 줄바꿈 및 큰 글자 UI 개선
* 플로우 선택 시 진행률이 줄어드는 현상 및 비동기 처리 중 뒤로가기 오류 수정
* 날짜 선택 범위와 바텀시트 UI 개선
* 탭루트 지갑 내보내기 문구 및 서명 정보 표시 개선

## mainnet 3.0.0, regtest 5.0.0 - 2026-06-18
### Added
* 탭루트 상속 지갑

## mainnet 2.0.3, regtest 4.0.3 - 2026-02-06
### Fixed
1. 지갑 보유 상태에서 기기 비번 해제 후 재설정 감지 > 데이터 삭제 > 지갑 추가 후 지갑 목록 잘못 나오는 버그 수정
2. 니모닉 앱에서 생성 / 다중 서명 지갑 > name/icon setup 화면에서 뒤로간 후 재진행 시 버그 수정
3. UI 수정 (카메라 아이콘, 버튼 테두리)

## mainnet 2.0.2, regtest 4.0.2 - 2026-01-22
### Fixed
* 안드로이드 보안 폴더에서 볼트 사용 > 보안 영역 접근 시 생체인증하면 접근 권한 갱신이 안됨 > PIN/Pattern/Passcode 요청하는 UX 추가
* 서명 전용 모드 > 안전 저장 모드로 전환 후 앱 재실행 시 지갑 로드 실패하는 버그
* 갤럭시 폴드 넓은 화면에서 QR View 개선
* 카메라 권한 '허용 안함'일 때 안내 개선

## mainnet 2.0.1, regtest 4.0.1 - 2026-01-12
### Fixed
* 학습용 볼트 '보기 전용 지갑으로 내보내기' - 코코넛 월렛 학습용 아이콘으로 보이도록 수정
* 다중 서명 지갑 hww 월렛 서명 시 가이드 개선
* 코드 리팩토링

## mainnet 2.0.0, regtest 4.0.0 - 2026-01-10
### Fixed
...
### Added
* 언어 - 일본어 추가
* 패스프레이즈에 특수문자 입력 시 경고문구 보여줌

## mainnet(beta) 0.1.1 - 2025-10-01
### Added
* 주사위 던져서 니모닉 만들기
* 니모닉 보이는 화면 경고문구 추가
